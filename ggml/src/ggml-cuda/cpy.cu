...
// q1_t dequant returns ADJACENT pairs (qr==1); the generic cpy_blck_q_f32
// assumes the q4-style split layout (j and j+qk/2), so it gets its own blck.
static __device__ void cpy_blck_q1_t_g128_f32(const char * cxi, char * cdsti) {
    float * cdstf = (float *) cdsti;

#pragma unroll
    for (int j = 0; j < QK1_T_g128; j += 2) {
        float2 dq;
        dequantize_q1_t_g128(cxi, 0, j, dq);
        cdstf[j + 0] = dq.x;
        cdstf[j + 1] = dq.y;
    }
}

static void ggml_cpy_f32_q1_t_g128_cuda(
    const char * cx, char * cdst, const int64_t ne,
    const int64_t ne00, const int64_t ne01, const int64_t ne02, const int64_t nb00, const int64_t nb01, const int64_t nb02,
    const int64_t nb03, const int64_t ne10, const int64_t ne11, const int64_t ne12, const int64_t nb10, const int64_t nb11, const int64_t nb12, const int64_t nb13, cudaStream_t stream) {
...
static void ggml_cpy_q1_t_g128_f32_cuda(
    const char * cx, char * cdst, const int64_t ne,
    ...
    const int64_t num_blocks = (ne/QK1_T_g128 + CUDA_CPY_BLOCK_SIZE - 1) / CUDA_CPY_BLOCK_SIZE;
    GGML_ASSERT(num_blocks <= INT_MAX);
    cpy_q_f32<cpy_blck_q1_t_g128_f32, QK1_T_g128><<<num_blocks, CUDA_CPY_BLOCK_SIZE, 0, stream>>>(
...
void ggml_cuda_cpy(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, ggml_tensor * src1) {
    ...
    if (src0->type == src1->type && contiguous_srcs) {
        ...
        ggml_cpy_q4_0_f32_cuda
                (src0_ddc, src1_ddc, ne, ne00, ne01, ne02, nb00, nb01, nb02, nb03, ne10, ne11, ne12, nb10, nb11, nb12, nb13, main_stream);
    } else if (src0->type == GGML_TYPE_F32 && src1->type == GGML_TYPE_Q1_T_g128) {
        ggml_cpy_f32_q1_t_g128_cuda
                (src0_ddc, src1_ddc, ne, ne00, ne01, ne02, nb00, nb01, nb02, nb03, ne10, ne11, ne12, nb10, nb11, nb12, nb13, main_stream);
    } else if (src0->type == GGML_TYPE_Q1_T_g128 && src1->type == GGML_TYPE_F32) {
        ggml_cpy_q1_t_g128_f32_cuda
                (src0_ddc, src1_ddc, ne, ne00, ne01, ne02, nb00, nb01, nb02, nb03, ne10, ne11, ne12, nb10, nb11, nb12, nb13, main_stream);
