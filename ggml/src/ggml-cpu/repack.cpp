...
//
// A q1_t_g128 byte packs 5 trits in base 3 (byte = d0 + 3*d1 + 9*d2 + 27*d3
// + 81*d4, each digit in {0,1,2} meaning trit-1), so a byte has only 3^5 =
// 243 possible values. For a FIXED 5-element window of Q8_0 activations,
// the partial dot product sum_k (d_k-1)*y[off+k] therefore also has only
// 243 possible values -- build them ONCE per activation block and every
// output row's byte becomes a single table lookup instead of a 5-trit
// unpack (mulhi digit extraction + vpshufb scatter). The table-build cost
// is shared across every row that reuses the same activation block, so
// this amortisation only pays off at the GEMV/GEMM level (which sees the
// whole weight matrix against one activation vector) -- vec_dot only ever
// sees one row and cannot amortise anything.
//
// A 128-weight block's 26 bytes cover 130 trit slots for 128 real weights
// (the last byte's final 2 slots are always-zero padding -- see
// block_q1_t_g128's comment in ggml-common.h). Weights dot-product against
// FOUR separate 32-wide Q8_0 sub-blocks, each with its own activation
// scale, so a byte's 5-weight window is only safe to fold into ONE table
// lookup if the whole window falls inside a single 32-weight sub-block.
// Bytes 6, 12, and 19 straddle a sub-block boundary -- verified against the
// AVX2 vec_dot kernel's ggml_q1_t_g128_shuffle_0..3 mask tables (arch/x86
// /quants.c): byte 6 digits 0-1 feed sub-block 0 and digits 2-4 feed
// sub-block 1; byte 12 digits 0-3 feed sub-block 1 and digit 4 feeds
// sub-block 2; byte 19 digit 0 feeds sub-block 2 and digits 1-4 feed
// sub-block 3. Each straddling byte is split into two segments, one per
// side of the boundary. Byte 25 has only 3 real digits (the other two are
// the always-zero padding). Every other byte is a single 5-digit segment.
// 26 bytes therefore decompose into 29 (byte, sub-block) segments -- this
// table was generated by an exhaustive weight-index walk (weight = 5*byte
// + digit, sub-block = weight/32) and cross-checked against the shuffle
// masks above by hand for all three straddle cases before being hardcoded.
// Portable (non-AVX2) apply: prebuilt tables for ONE 128-weight activation
// block, applied to ONE group of 8 interleaved rows, ADDING each row's
// contribution into out8[0..7] (callers sum across activation blocks / `l`
// iterations). q1t_gemv_segment / q1t_gemv_segments / q1t_build_all_tables
// are shared with the AVX2 path via repack.h.
static inline void q1t_apply_tables_to_group(
        float out8[8],
        const int32_t tables[Q1T_GEMV_NUM_SEGMENTS][243],
        const float yscale[4],
        const block_q1_t_g128x8 * b) {
    int32_t dot[8][4];
    ...
    for (int s = 0; s < Q1T_GEMV_NUM_SEGMENTS; s++) {
        ...
        const int                sb    = seg->subblock;
        for (int row = 0; row < 8; row++) {
            dot[row][sb] += table[col[row]];
        }
    }

    for (int row = 0; row < 8; row++) {
        const float wscale = GGML_CPU_FP16_TO_FP32(b->d[row]);
        float       total  = 0.0f;
        for (int sb = 0; sb < 4; sb++) {
            total += (float) dot[row][sb] * yscale[sb];
        }
        out8[row] += wscale * total;
    }
}

static int repack_q1_t_g128_to_q1_t_g128_8_bl(struct ggml_tensor * t, int interleave_block,
                                               const void * GGML_RESTRICT data, size_t data_size) {
    GGML_ASSERT(t->type == GGML_TYPE_Q1_T_g128);
    GGML_UNUSED(interleave_block); // no sub-byte interleave choice -- qs[] is a plain byte-transpose, like q8_0's blck_size_interleave==1 path
    constexpr int nrows_interleaved = 8;

    block_q1_t_g128x8 *     dst     = (block_q1_t_g128x8 *) t->data;
    const block_q1_t_g128 * src     = (const block_q1_t_g128 *) data;
    int                     nrow    = ggml_nrows(t);
    int                     nblocks = t->ne[0] / QK1_T_g128;

    GGML_ASSERT(data_size == (size_t) nrow * nblocks * sizeof(block_q1_t_g128));
    ...
    for (int b = 0; b < nrow; b += nrows_interleaved) {
        for (int64_t x = 0; x < nblocks; x++) {
            for (int r = 0; r < nrows_interleaved; r++) {
                const block_q1_t_g128 * row = &src[x + (int64_t) r * nblocks];
                dst->d[r] = row->d;
                for (int byte_idx = 0; byte_idx < 26; byte_idx++) {
                    dst->qs[byte_idx * 8 + r] = row->qs[byte_idx];
                }
            }
            dst++;
        }
        src += (int64_t) nrows_interleaved * nblocks;
    }
    return 0;
}
...
void ggml_gemv_q1_t_g128_q8_0_generic(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, const void * GGML_RESTRICT vy, int nr, int nc) {
    const int nb_w = n / QK1_T_g128; // weight blocks per row (128-wide); also the activation-block ("l") loop range
    const int nb_a = n / QK8_0;      // activation Q8_0 blocks per row (32-wide) = 4*nb_w
    const int ncols_interleaved = 8;
    ...
    for (int y = 0; y < nr; y++) {
        ...
        // Outer loop over activation blocks, inner loop over weight columns:
        // this ordering is what lets the 29 segment tables (built once per
        // `l`) be reused across every column group instead of every row
        // re-paying the table-build cost.
        for (int l = 0; l < nb_w; l++) {
            ...
            for (int64_t x = 0; x < nc / ncols_interleaved; x++) {
                ...
                q1t_apply_tables_to_group(&srow[x * ncols_interleaved], tables, yscale, b);
            }
        }
    }
}
...
void ggml_gemm_q1_t_g128_q8_0_generic(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, const void * GGML_RESTRICT vy, int nr, int nc) {
    ...
    // De-interleave each group of 4 activation rows back into plain,
    // contiguous block_q8_0 arrays (one per row), then reuse the exact same
    // per-row table-build-and-apply core as GEMV. Correctness-preserving
    // composition rather than a bespoke 4-row-batched kernel: the index
    // formula below is copied verbatim from ggml_gemm_q4_0_4x8_q8_0_generic
    // / ggml_gemm_q4_0_8x8_q8_0_generic's proven-correct block_q8_0x4 read
    // pattern (qk=32, blocklen=8: e<16 reads offset k*32+m*8+i, e>=16 reads
    // the same plus 64), not re-derived from scratch.
...
template <> int repack<block_q1_t_g128, 8, 8>(struct ggml_tensor * t, const void * data, size_t data_size) {
    return repack_q1_t_g128_to_q1_t_g128_8_bl(t, 8, data, data_size);
}
...
template <> void gemv<block_q1_t_g128, 8, 8, GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    ggml_gemv_q1_t_g128_q8_0(n, s, bs, vx, vy, nr, nc);
}
...
template <> void gemm<block_q1_t_g128, 8, 8, GGML_TYPE_Q8_0>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    ggml_gemm_q1_t_g128_q8_0(n, s, bs, vx, vy, nr, nc);
}
...
static const ggml::cpu::tensor_traits * ggml_repack_get_optimal_repack_type(const struct ggml_tensor * cur) {
    ...
    // instance for Q1_T_g128
    static const ggml::cpu::repack::tensor_traits<block_q1_t_g128, 8, 8, GGML_TYPE_Q8_0> q1_t_g128_8x8_q8_0;
    ...
    } else if (cur->type == GGML_TYPE_Q1_T_g128) {
        // GEMV/GEMM-level 243-value lookup table amortisation (see the
        // block comment above ggml_gemv_q1_t_g128_q8_0_generic); only
        // wired up for AVX2 so far. NB_COLS=8 needs ne[1] a multiple of 8;
        // ne[0] % QK1_T_g128(128) == 0 is already a hard requirement of the
        // format itself, enforced elsewhere at quantize time.
        if (ggml_cpu_has_avx2()) {
            if (cur->ne[1] % 8 == 0) {
                return &q1_t_g128_8x8_q8_0;
            }
        }
    }
