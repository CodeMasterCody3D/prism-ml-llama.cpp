...
void ggml_vec_dot_q1_0_g128_q8_0(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx, const void * GGML_RESTRICT vy, size_t by, int nrc) {
    // The NEON body that used to live here decoded the OLD plain-Q1_0 format:
    // 1 bit per weight, "bit=1 -> +1, bit=0 -> -1". Q1_0_g128 is 2 bits per
    // weight ({0,1,2} -> {-1,0,+1}, 4 per byte), so that kernel read the wrong
    // stride and produced garbage on every ARM machine. Delegate to the correct
    // generic implementation, exactly as g32/g64 above and as x86 does, until a
    // real 2-bit ternary NEON kernel (vdotq_s32) is written.
