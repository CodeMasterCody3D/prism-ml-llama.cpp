...
#undef CREATE_MM
    ...
    for (uint32_t w = 0; w < DMMV_WG_SIZE_COUNT; ++w) {
        ...
        for (uint32_t i = 0; i < mul_mat_vec_max_cols; ++i) {
            ...
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f32_f32[w][GGML_TYPE_Q1_0_g128][i], "mul_mat_vec_q1_0_g128_f32_f32", arr_dmmv_q1_0_g128_f32_f32_len[reduc], arr_dmmv_q1_0_g128_f32_f32_data[reduc], "main", mul_mat_vec_num_bindings, sizeof(vk_mat_vec_push_constants), {1*rm_stdq, 1, 1}, {wg_size_subgroup, 1*rm_stdq, i+1}, 1, true, use_subgroups, force_subgroup_size);
            ...
            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_f16_f32[w][GGML_TYPE_Q1_0_g128][i], "mul_mat_vec_q1_0_g128_f16_f32", arr_dmmv_q1_0_g128_f16_f32_len[reduc], arr_dmmv_q1_0_g128_f16_f32_data[reduc], "main", mul_mat_vec_num_bindings, sizeof(vk_mat_vec_push_constants), {1*rm_stdq, 1, 1}, {wg_size_subgroup, 1*rm_stdq, i+1}, 1, true, use_subgroups, force_subgroup_size);
            ...
            {
                // Ternary integer mmvq: plain int32 math, needs neither
                // integer-dot glslc support nor the device acceleration bit.
                const uint32_t subgroup_size_int = (device->vendor_id == VK_VENDOR_ID_INTEL && device->subgroup_size_control) ? device->subgroup_min_size : device->subgroup_size;
                const uint32_t wg_size_subgroup_int = (w == DMMV_WG_SIZE_SUBGROUP) ? subgroup_size_int : (subgroup_size_int * 4);

                ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_q8_1_f32[w][GGML_TYPE_Q1_0_g128][i], "mul_mat_vec_q1_0_g128_q8_1_f32", arr_dmmv_q1_0_g128_q8_1_f32_len[reduc], arr_dmmv_q1_0_g128_q8_1_f32_data[reduc], "main", mul_mat_vec_num_bindings, sizeof(vk_mat_vec_push_constants), {1*rm_stdq_int, 1, 1}, {wg_size_subgroup_int, 1*rm_stdq_int, i+1}, 1, true, use_subgroups, subgroup_size_int);
            }
        }
        ...
        ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_f32[w][GGML_TYPE_Q1_0_g128], "mul_mat_vec_id_q1_0_g128_f32", arr_dmmv_id_q1_0_g128_f32_f32_len[reduc], arr_dmmv_id_q1_0_g128_f32_f32_data[reduc], "main", mul_mat_vec_id_num_bindings, sizeof(vk_mat_vec_id_push_constants), {1*rm_stdq, 1, 1}, {wg_size_subgroup, 1*rm_stdq}, 1, true, use_subgroups, force_subgroup_size);
        ...
        {
            // Ternary integer mmvq (id): plain int32 math, needs neither
            // integer-dot glslc support nor the device acceleration bit.
            const uint32_t subgroup_size_int = (device->vendor_id == VK_VENDOR_ID_INTEL && device->subgroup_size_control) ? device->subgroup_min_size : device->subgroup_size;
            const uint32_t wg_size_subgroup_int = (w == DMMV_WG_SIZE_SUBGROUP) ? subgroup_size_int : (subgroup_size_int * 4);

            ggml_vk_create_pipeline(device, device->pipeline_dequant_mul_mat_vec_id_q8_1_f32[w][GGML_TYPE_Q1_0_g128], "mul_mat_vec_id_q1_0_g128_q8_1_f32", arr_dmmv_id_q1_0_g128_q8_1_f32_len[reduc], arr_dmmv_id_q1_0_g128_q8_1_f32_data[reduc], "main", mul_mat_vec_id_num_bindings, sizeof(vk_mat_vec_id_push_constants), {1*rm_stdq_int, 1, 1}, {wg_size_subgroup_int, 1*rm_stdq_int}, 1, true, use_subgroups, subgroup_size_int);
        }
    }
    ...
    ggml_vk_create_pipeline(device, device->pipeline_dequant[GGML_TYPE_Q1_0_g128], "dequant_q1_0_g128", dequant_q1_0_g128_len, dequant_q1_0_g128_data, "main", 2, 5 * sizeof(uint32_t), {256 * 16, 1, 1}, {}, 1);
...
static bool ggml_vk_should_use_mmvq(const vk_device& device, uint32_t m, uint32_t n, uint32_t k, ggml_type src0_type) {
    ...
    // Ternary q1_0_g128: the integer mmvq is exact signed-ternary math (no
    // fp16 dequant round-trip) and does not need integer-dot acceleration.
    if (src0_type == GGML_TYPE_Q1_0_g128) {
        return true;
    }
...
static void ggml_vk_mul_mat_vec_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const struct ggml_cgraph * cgraph, int node_idx) {
    ...
    // Ternary q1_0_g128 has a plain-integer mmvq that does not depend on the
    // device's integer-dot acceleration, so it qualifies unconditionally.
    bool quantize_y = (ctx->device->integer_dot_product || src0->type == GGML_TYPE_Q1_0_g128) && src1->type == GGML_TYPE_F32 && ggml_is_contiguous(src1) && !y_non_contig && (ne11 * ne10) % 4 == 0 && ggml_vk_should_use_mmvq(ctx->device, ne01, ne11, ne10, src0->type);
...
static void ggml_vk_mul_mat_vec_id_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const struct ggml_cgraph * cgraph, int node_idx) {
    ...
    const bool f16_f32_kernel = src1->type == GGML_TYPE_F32;
    // See ggml_vk_mul_mat_vec_q_f16: ternary q1_0_g128 has a plain-integer mmvq.
    bool quantize_y = (ctx->device->integer_dot_product || src0->type == GGML_TYPE_Q1_0_g128) && src1->type == GGML_TYPE_F32 && ggml_is_contiguous(src1) && !y_non_contig && (ne11 * ne10) % 4 == 0 && ggml_vk_should_use_mmvq(ctx->device, ne01, ne12, ne10, src0->type);
...
// The Vulkan backend has shaders for Q1_0_g128 (the production ternary type)
// but not for the rest of the Q1_0 family. All type switches below already
// fall through to unsupported for the others, but refuse them explicitly up
// front so the scheduler reliably falls back to the CPU for any op touching
// these types, even if a generic quantized path is added later.
